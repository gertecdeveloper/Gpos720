package com.example.exemplogpos720_kot.ExemploMifare
...
import com.example.exemplogpos720_kot.Utils
...
class ExemploMifare : AppCompatActivity() {
    ...
    fun onReadBlocks(view: View) {
        var blocksToRead = Integer.parseInt(readBlocksText.text.toString())
        blocksToRead = Math.min(blocksToRead, 64)
        readBlocks(blocksToRead)
    }

    private fun readBlocks(blocksToRead: Int) {
